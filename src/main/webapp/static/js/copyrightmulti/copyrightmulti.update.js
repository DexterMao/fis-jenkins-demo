$(function() {
	videojs(document.getElementsByClassName('video_class')[0], {}, function() {});
	
	checkType($('input[name="type"]:checked').val());
	checkpayType($('input[name="payType"]:checked').val());
	init($('#picturepath').val());
	// 上传图片
	$(".upload_img").on(
			"click",
			"li:not('.ico_del')",
			...
			function(e) {
				...
				xhr.onload = function() {
					if (xhr.status === 200) {
						...
						$('#path').val(obj.path);
						$("#pathSource").html('<video class="video-js vjs-default-skin video_class" controls preload="none" width="480" height="272" poster="">'
                                +'<source src="'+obj.path+'" type="video/mp4" id="" />'
                            +'</video>');
						videojs(document.getElementsByClassName('video_class')[0], {}, function() {});
					} else {
						console
...
function sava(canEdit) {
	if (canEdit) {
		layer.msg('视频处于有效期内，不能修改！！', {
			time : 1500
		})
	}
	var copyrightmultiId = $('#COPYRIGHTMULTI_ID').val();
	...
	var params = {
		copyrightmultiId : copyrightmultiId,
		type : type,
		mediaName : mediaName,
	...
	kakaAjax('/copyrightmulti/updateCM.json', params, function(data) {
