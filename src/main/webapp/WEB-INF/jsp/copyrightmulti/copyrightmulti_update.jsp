	...
	<div class="kks_main">
		...
		<div class="m_right">
			...
			<ul class="nav nav-tabs m_r_tab">
				<li class="active"><a href="javascript:;">修改多媒体</a></li>
			</ul>
			...
			<div class="kks_content">
				...
				<div class="form-horizontal">
					<input type="hidden" name="COPYRIGHTMULTI_ID"
						id="COPYRIGHTMULTI_ID" value="${cm.copyrightmultiId}" />
					<div class="form-group">
						<label for="" class="col-xs-2 control-label">多媒体类型：</label>
						<div class="col-xs-8">
							<label class="radio-inline m-r20"><input type="radio"
								name="type" id="type1" value="01" onclick="checkType('01');"
								${cm.type=="01"?"checked":"disabled"} > 视频</label> <label
								class="radio-inline m-r20"><input type="radio"
								name="type" id="type2" value="02" onclick="checkType('02');"
								${cm.type=="02"?"checked":"disabled"} > 音频</label>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-xs-7">
							<input type="text" class="form-control" placeholder="最多输入32个字符"
								id="mediaName" maxlength="32" value="${cm.mediaName}">
						</div>
					</div>
							...
							<div class="col-xs-3">
								<select class="form-control" id="videoCategoryId">
									...
									<c:forEach items="${categories}" var="v">
										<option value="${v.videoCategoryId}"
											${v.videoCategoryId == cm.videoCategoryId?"selected":""}>${v.categoryName }</option>
									</c:forEach>
								</select>
							</div>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-xs-7">
							<input type="text" class="form-control" placeholder="多个标签用，隔开.每个最多10字"
								id="labelStr" value="${cm.labelStr }">
						</div>
					</div>
					<div class="form-group">
						<label for="" class="col-xs-2 control-label">关键字：</label>
						<div class="col-xs-7">
							<input type="text" class="form-control" placeholder="请输入关键字,最多64字"
								id="keywords" value="${cm.keywords }" maxlength="64">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-xs-8">
							<label class="radio-inline m-r20"><input type="radio"
								name="payType" id="type11" value="0"
								onclick="checkpayType('0');" ${cm.payType=="0"?"checked":""}>
								免费</label> <label class="radio-inline m-r20"><input type="radio"
								name="payType" id="type12" value="1"
								onclick="checkpayType('1');" ${cm.payType=="1"?"checked":""}>
								收费</label>
						</div>
					</div>
						...
						<div class="col-xs-3">
							<div class="input-group">
								<input type="number" class="form-control" placeholder="码商所得佣金"
									id="productConcession" value="${cm.productConcession}">
					...
					<div class="form-group">
						<label for="" class="col-xs-2 control-label">上传图片：</label> <input
							type="hidden" name="picturepath" id="picturepath"
							value="${cm.picturepath }" placeholder="图片路径" title="图片路径"
							readonly="readonly" />
						<div class="col-xs-8">
							<ul class="upload_img">
					...
					<div class="form-group" id="type_video">
						<label for="" class="col-xs-2 control-label">上传视频：</label>
						 <input type="hidden"
								name="path" id="path" value="${cm.path }" placeholder="多媒体路径"
								readonly="readonly" />
						<div class="col-xs-3">
					...
					<div class="form-group" id="type_music">
						...
						<input type="hidden" name="audioPath" id="audioPath"
								readonly="readonly" value="${cm.path }" />
						<div class="col-xs-3">
							<input type="file" id="audio" class="form-control" accept=".mp3" />
						</div>
						<p class="col-gray">格式支持：mp3</p>
					</div>
					<input type="hidden" id="input_progressbar" value="100.0">
					...
					<div class="form-group">
						...
						<div class="col-xs-8">
							<div class="form-control-static" id="pathSource">
								<video class="video-js vjs-default-skin video_class"
									controls preload="none" width="480" height="272"
									poster="">
									<source src="${cm.path }" type='video/mp4' id="" />
								</video>
							</div>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-xs-7">
							<textarea name="" class="form-control" rows="5"
								placeholder="最多输入2000个字符" id="descr" maxlength="2000">${cm.descr}</textarea>
						</div>
					</div>
					<div class="form-group">
						<div class="col-xs-offset-2 col-xs-7">
							<button type="button" class="btn btn-primary pull-left f-s14"
								onclick="sava(${canEdit});">保存</button>
						</div>
					</div>
...
<script type="text/javascript"
	src="/static/js/copyrightmulti/copyrightmulti.update.js"></script>
