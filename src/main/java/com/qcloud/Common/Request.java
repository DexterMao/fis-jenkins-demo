...
public class Request {
	...
	protected static int	timeOut		= 30000;			// 设置连接主机的超时时间，单位：毫秒，可以根据实际需求合理更改 timeOut 的值。
	...
	public static String sendRequest(String url, Map<String, Object> requestParams, String requestMethod, String fileName) {
		...
		try {
			...
			e.printStackTrace();
			result = "{\"code\":-2700,\"location\":\"com.qcloud.Common.Request:225\",\"message\":\"api sdk throw exception! " + e.getMessage() + "\"}";
		} finally {
			// 使用finally块来关闭输入流
			try {
				if (in != null) {
				...
				e2.getMessage();
				result = "{\"code\":-2800,\"location\":\"com.qcloud.Common.Request:234\",\"message\":\"api sdk throw exception! " + e2.getMessage() + "\"}";
			}
		}
		rawResponse = result;
		return result;
